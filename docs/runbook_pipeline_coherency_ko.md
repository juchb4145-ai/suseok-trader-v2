...
5. `HISTORICAL_CLOSED`는 별도 승인된 disposition preview/apply만 사용하고, `MISSING_CANDIDATE_MANUAL_REVIEW`와 `STALE_OTHER_DATE_MANUAL_REVIEW`는 수동 검토 상태로 유지한다.
...
evidence가 남았다고 가정하지 않는다. 현재 운영 RCA의 `ACTIVE_CURRENT=0`이면 강제 실행하지 않는다.
