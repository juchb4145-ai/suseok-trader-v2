...
| `market_cross_exchange_observations` | KRX/NXT regular 1분 교차시장 관찰치. `(code, bucket_start)` 기준 |
...
## KRX/NXT Cross-Exchange Observation

regular session tick이 projection될 때 `(code, 1분 bucket)` 기준으로 KRX와 NXT bar를 읽어 `market_cross_exchange_observations`를 갱신한다.

| 값 | 의미 |
| --- | --- |
| `krx_last_price`, `nxt_last_price` | 해당 1분 bucket의 KRX/NXT 마지막 가격 |
| `divergence_bp` | `(nxt_last_price - krx_last_price) / krx_last_price * 10000` |
| `krx_volume_share`, `nxt_volume_share` | 양시장 volume delta 합계 대비 각 시장 비중 |
| `krx_tick_count`, `nxt_tick_count` | 양시장 1분 tick count |

한쪽 시장만 있으면 row는 남기되 `divergence_bp`는 `null`이다. 이 값은 체결 품질과 시장 미세구조를 보기 위한 관찰 컨텍스트이며 주문 경로, 매수 신호, `MATCHED_OBSERVATION`, `OBSERVE_PASS`의 의미를 바꾸지 않는다.
...
| Risk Gate | market freshness, liquidity, spread, VWAP extension, opt-in cross-exchange divergence |
