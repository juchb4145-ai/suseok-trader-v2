...
3. 한 subject씩 preview하고 CAS와 expected action을 검증한다.
4. 별도 apply 승인이 있을 때만 한 subject씩 append-only disposition 후 strict verify한다.
...
| Phase C1 | authoritative manual evidence와 한 subject apply | 9건 일괄 처리, C2 대상 처리 |
