...
구체적인 private manifest, alias별 strict preflight, caller-owned transaction,
동일 request reconcile 및 U38 completion handoff 절차는
`docs/runbook_fast0_incremental_dead_letter_campaign_ko.md`를 따른다. 일반
`tools.resolve_incremental_evaluation_dead_letter`의 dispose/reset/sweep 경로를 이
캠페인의 대체 수단으로 사용하지 않는다.
