...
허용 가능한 timing 후보는 `GOOD_PULLBACK`, `PULLBACK_RECLAIM`, `VWAP_RECLAIM`이다. 이 상태도 바로 주문으로 이어지지 않고 `OrderPlanDraft` 초안으로만 남는다.
...
- `setup_type`: `THEME_LEADER_PULLBACK`, `VWAP_RECLAIM`, `BREAKOUT_RETEST`, `THEME_FOLLOWER_EXPANSION`, `NO_SETUP`
