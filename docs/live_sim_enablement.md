...
과거 FAST-0 evidence는 `RETIRED_HISTORICAL`로 보존하며 Preview가 이를 해결하거나 FAST-0
`PASS`로 승격하지 않는다. 현재 거래일 tick/index/context readiness가 `BLOCKED`이면 selectable
plan이 있더라도 `canary_ready=false`다. 자세한 계약은
...
## FAST-5 Guarded Automatic Canary

FAST-5는 기존 pilot/order-plan 경로 앞에 default-off 자동화 gate를 추가한다.
...
$env:LIVE_SIM_FAST5_AUTO_QUEUE_ENABLED = "false"
$env:LIVE_SIM_FAST5_AUTOMATIC_CANARY_ENABLED = "false"
$env:LIVE_SIM_NXT_SUPPORT_CONFIRMED = "false"
$env:LIVE_SIM_ORDER_EXCHANGE = "KRX"
