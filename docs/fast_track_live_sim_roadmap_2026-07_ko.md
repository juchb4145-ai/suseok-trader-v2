...
- 현재 거래일 pipeline 전수 inventory가 500행 이하이고 count coverage가 일치하며, 최신
  미만료 `PLAN_READY`가 1건 이상이고 그 전부의 FK/source run/watermark/current source/age가 PASS
- PASS plan 중 하나를 전체 주문 구동 필드 canonical SHA-256과 exact
  ...
  plan 재선택 없이 `send_order` enqueue 직전 같은 binding을 `BEGIN IMMEDIATE` 안에서 재검증.
