...
Post-Merge Qualification
  → Pure Preview
  → 승인 결속 자동 LIVE_SIM Bootstrap 1건
...
- FAST 승인 결속 bootstrap canary 차단 여부: **아님**
...
| Operational Bootstrap Canary | `BLOCKED` | schema 63 exact migration, U01~U03 당일 fence release, fresh COMPLETE broker snapshot, 현재 시장 `PLAN_READY`, 당일 승인 SHA | 자동 1회 LIVE_SIM lifecycle |
...
| FAST-5 Automatic Canary | `BLOCKED_BY_BOOTSTRAP` | Bootstrap + Alpha + Shadow + Broker reconcile | 기본 OFF guarded 자동화 구현, 운영 evidence 대기 |
...
sidecar가 없으면 SQLite URI `mode=ro&immutable=1`, 활성 WAL/SHM pair가 있으면 `mode=ro`로 열며
`PRAGMA query_only=ON`과 단일 deferred snapshot에서만 계산한다.
...
safety/reconcile/broker-boundary 상태에 따라 `false`일 수 있으며, `true`여도 Operational Bootstrap Canary나
주문 실행을 승인하지 않는다.
...
## 7. Operational Bootstrap Canary — 승인 결속 자동 LIVE_SIM 1건

### 목표

수익성을 다시 판단하지 않고 Kiwoom 모의투자 주문 경계를 한 건의 complete lifecycle로
검증한다. 사람이 HTS에서 수동 주문을 만들지 않고 FAST-5가 현재 eligible plan 하나를 선택한다.
...
- 당일 payload와 운영 승인이 같은 lowercase SHA-256으로 결속
...
queue 전에 승인 SHA를 append-only reservation으로 먼저 소비한다. 같은 SHA의 두 번째 신규 BUY를
생성하지 않으며 후속 cycle은 cancel/close-only `PROTECT_ONLY` lifecycle만 수행한다. 주문이
만들어지지 않았는데 다시 시도해야 하면 새 approval ID와 새 SHA를 별도로 승인한다.

### Evidence

```text
reports/fast_track/bootstrap_automatic_canary/<trade_date>/<timestamp>/
```
...
구현·실행 계약은 `docs/fast2a_point_in_time_alpha_replay_ko.md`를 따른다. 원본 SQLite를
직접 열지 않고 main+WAL을 격리 snapshot으로 복사한다. snapshot에 WAL이 없으면
`immutable=1`, WAL이 있으면 committed WAL까지 포함하는 `mode=ro`로 열며, 원본
main/WAL/SHM fingerprint가 실행 전후 같아야 운영 DB write 0으로 인정한다.
...
stable top 1 → 승인 결속 자동 LIVE_SIM bootstrap canary
```
...
activation: BLOCKED_BY_BOOTSTRAP
```

구현 계약은 `docs/fast5_guarded_automatic_canary_ko.md`를 따른다. status/preview는
read-only이고, bootstrap 실제 queue는 당일 공개 payload의 승인 SHA와 모든 동적 safety gate가
PASS일 때 한 번만 가능하다. 준비 미완료 gate 거절은 rollback latch를 남기지 않는다.
이번 개발 단계에서는 운영 DB, Gateway, LIVE_SIM 주문을 실행하지 않는다.

### 진입 조건

- FAST-0 `RETIRED_HISTORICAL` 결정이 기록되어 현재 gate에서 재사용되지 않음
- FAST-1 DONE
- FAST-4 broker reconcile PASS
- unresolved broker boundary/UNCONFIRMED 0
- pipeline coherency qualification PASS
- 당일 bootstrap approval SHA 승인

최초 1회 bootstrap 이후 정규 자동화 승격에는 다음이 추가로 필요하다.

- bootstrap complete lifecycle evidence PASS + SHA
- FAST-2B `ALPHA_QUALIFIED` + SHA
- FAST-3 치명적 fill disagreement 없음 + Shadow PASS SHA
...
- 해당 단계의 코드가 요구하는 current target schema exact 일치 (현재 fence-release 준비 target: schema 63)
- OBSERVE smoke PASS
- 주문 delta 0
- coherency/boundary/dead-letter blocker 0

### Gate S2 — Approved Automatic Bootstrap

- Preview delta 0 및 FAST-1 `canary_ready=true`
- 현재 거래일 pipeline 전수 inventory가 500행 이하이고 count coverage가 일치하며,
  candidate별 최신 EntryTiming evaluation이 정확히 가리키는 미만료 actionable
  `PLAN_READY`가 1건 이상이고 그 전부의 FK/source run/watermark/current source/age가 PASS.
  같은 candidate의 과거 idempotency key에 남은 superseded plan은 진단에는 보존하되 실행
  inventory와 gate blocker에서는 제외
- PASS plan 중 하나를 전체 주문 구동 필드 canonical SHA-256과 exact
  plan/entry/strategy/risk/source identity에 결속하고, 실행 경로에서 EntryTiming 재실행·일반
  plan 재선택 및 current source를 바꾸는 refresh/backfill 없이 `send_order` enqueue 직전 같은
  binding을 `BEGIN IMMEDIATE` 안에서 재검증.
  잠금 뒤 intent를 재조회하고 설정 계좌·LIMIT BUY·수량·가격·금액·만료·idempotency key까지
  terminal sizing과 일치해야 하며, 오류는 Operating/FAST-5 rollback latch까지 전파
- 당일 공개 approval payload와 별도 운영 승인이 같은 lowercase SHA-256으로 결속
- 과거 broker-boundary maintenance fence는 schema 63 append-only 당일 release로만 해제되며,
  승인 SHA는 운영 DB identity·app/schema·Gateway command snapshot과 일치
- fresh COMPLETE broker snapshot과 runtime/order safety gate PASS; snapshot 조회는 fence release
  승인과 분리된 운영 승인 단계
- `phase=BOOTSTRAP`, `ready=true`에서 approval SHA당 자동 LIVE_SIM 신규 BUY 정확히 1건
- complete lifecycle evidence PASS 및 evidence SHA 결속
- reconcile PASS, UNCONFIRMED 0, LIVE_REAL command 0
