...
4. DB main identity를 Windows deny-write/delete handle에 pin하고 sidecar/writer 부재를 확인한다.
5. immutable `mode=ro`, `query_only`, 단일 deferred snapshot에서 `quick_check(1)`, app/schema identity,
   schema manifest와 runtime lock 0을 확인한다.
6. 기존 orphan ledger 전체의 legacy generic/invalid JSON/noncanonical JSON/invalid row/chain count가
   모두 0인지 aggregate-only로 감사한다.
7. 선택 alias 한 건의 current orphan preview가 `ELIGIBLE`, `CANDIDATE_ABSENT`, `SAFE_DB_PROVEN`,
   reason 0이고 manifest six-part CAS와 exact match하는지 확인한다.
8. DB, manifest, evidence와 broker artifact를 다시 확인해 전체 구간 content/identity가 변하지 않았음을
   증명한다. 동일 bytes로 파일을 교체해도 identity drift로 거부한다.
9. 원 ID/path/provenance 본문을 제거한 aggregate report만 쓴다.
...
실제 ledger의 `evidence_json`은 이 safe binding을 `fast0-pipeline-orphan-apply-binding.v1`으로
...
R8 preflight는 DB file-level 기준선에 결속되므로 어떤 승인 write 뒤에도 기존 report는 신규 request에
재사용할 수 없다. 현재 Phase B dead-letter disposition을 먼저 수행하면 R6/R7 base fingerprint가
달라진다. 따라서 Phase B 이후 Phase C를 진행하려면 R9가 생성하는 predecessor apply/completion report
chain과 post-state fingerprint를 R8에 전이 결속하는 별도 handoff 계약이 먼저 병합돼야 한다. 이 handoff
없이 단계 순서를 바꾸거나 stale R8 report를 재사용하지 않는다.
