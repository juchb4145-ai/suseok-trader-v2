...
- DRY_RUN OMS: PR10부터 internal simulation status, recent intents/orders, and paper positions를
  read-only로 표시한다.
- DRY_RUN Exit: PR11부터 exit engine status, recent exit evaluations, and exit signals를
  read-only로 표시한다.
...
PR11부터 같은 `dry_run` section 아래에 `exit_engine` subsection이 추가된다. 표시 항목은 exit
engine enable flags, intent/order/fill enable flags, broker/gateway/live 금지 플래그,
evaluation/signal/exit-intent/exit-order/exit-execution count, recent exit evaluations, recent exit
signals, recent exit orders, and warnings이다. STOP_LOSS, TAKE_PROFIT, TRAILING_STOP, MAX_HOLD,
DATA_STALE_EXIT_CAUTION, THEME_WEAKENING, RISK_DETERIORATION, STRATEGY_INVALIDATED 같은 signal은
badge와 테이블 row로만 표시한다.
...
modify, exit intent creation, exit order creation, or exit simulated fill 버튼이 없다. DRY_RUN API와
DRY_RUN Exit API의 POST endpoints는 CLI/API 수동 조작용이며 Dashboard에서 호출하지 않는다.
