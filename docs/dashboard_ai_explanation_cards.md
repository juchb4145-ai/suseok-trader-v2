...
LIVE_SIM review 카드는 추가로 `live_sim_review_id`, `live_sim_order_id`, `reconcile_id`,
`review_only=true`, `order_action_allowed=false`, `gateway_command_allowed=false`,
`live_real_allowed=false`를 포함한다.
...
- Dashboard JavaScript의 LIVE_SIM review POST 호출
...
## Codex Prompt Draft Cards

PR AI-5 adds `CODEX_PROMPT_DRAFT` cards. These cards show stored prompt draft summaries,
acceptance criteria, forbidden scope, test plan, and a read-only prompt text area.

The UI may provide a browser clipboard copy button labeled `복사` or `텍스트 복사`. This is not a
Codex execution button, not a GitHub write action, and not an API POST. Dashboard JavaScript must
continue to use `GET /api/dashboard/snapshot` and `GET /api/dashboard/ai-explanations*` only for
display.

Forbidden labels remain absent from Dashboard controls: `Codex 실행`, `PR 생성`, `브랜치 생성`,
`커밋`, `푸시`, `자동 수정`, and `적용`.

## LIVE_SIM Review Cards

PR AI-6 adds LIVE_SIM review cards sourced from `ai_live_sim_review_reports`. These cards show
root cause category, severity, suggested checks, warnings, report sections, and links to related
LIVE_SIM order or reconcile IDs.

Dashboard may show these cards in the AI Review section and summarize latest reports in the
LIVE_SIM panel. It must not expose review generation, AI run, retry, cancel, modify, reconcile
command, or order execution controls.
