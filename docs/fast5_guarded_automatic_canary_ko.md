...
읽어 `CREATED` 상태를 확인하고, 설정 계좌·LIMIT BUY·수량·가격·금액·만료·idempotency key를
terminal sizing과 exact 비교한 뒤에만 payload를 만든다. 한 필드라도 바뀌면 intent를
`REJECTED`로 기록하고 Gateway command와 LIVE_SIM order를 만들지 않는다. 이 terminal 오류는
Pilot에서 Operating/FAST-5 rollback latch까지 실패로 전파한다.
...
`side_effects_unknown=true`, `no_order_side_effects=false`로 기록한다. 이전 v1/v2/v3 latch도
exact-plan terminal binding을 추가한 v4에서 그대로 유효하다.
...
- gate 이후 동일 plan ID의 가격·수량·lineage가 바뀌어도 terminal binding mismatch로
  Gateway command/LIVE_SIM order `0`
