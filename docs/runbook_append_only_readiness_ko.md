...
4. lifecycle worker와 market-data/reference/index/regime/scan reconcile을 persist한다.
5. 당일 여섯 component가 모두 qualified인지, command/order-command delta가 0인지 확인한다.
6. session sidecar 대비 FAILED/order-command delta가 0인지 확인한다.
7. 누적 readiness와 일별 JSON/Markdown 보고서를 남긴다.
