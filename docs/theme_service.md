...
PR-2부터 `services/theme_leadership`가 이 계층 위에 추가된다. Theme Leadership Service는 기존 membership과 Market Data projection을 읽어 실시간 주도 테마 ranking, RT-TLS 전용 state/role, watchset을 계산한다. 기존 Theme Service DB/API를 대체하지 않으며, `LEADING`/`SPREADING` theme과 `LEADER`/`CO_LEADER` role은 매수 신호가 아니다.
...
| 하지 않는 일 | RT-TLS watchset을 자동 주문이나 LIVE_SIM auto pipeline으로 직접 승격 |
