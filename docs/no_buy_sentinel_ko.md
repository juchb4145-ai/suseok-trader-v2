...
`reason_summary`는 기존 `reason_counts`, `stage_counts`, `block_type_counts`에 더해
`channel_counts`와 `channel_reason_counts`를 제공한다.

- `BLOCKING`: safety/config/risk/limit처럼 현재 주문 경로를 막는 reason
- `WAITING`: order plan 미생성, data wait, threshold miss처럼 재평가 가능한 대기 reason
- `INFO`: `MARKET_TICK_FRESH`, `THEME_FOLLOWER_MEMBER`처럼 원인 설명에는 유용하지만 차단으로
  집계하지 않는 정보성 reason

`top_near_miss[*].reason_channels`도 같은 3채널로 reason을 나누어 보여준다.
`CANDIDATE_WITHOUT_ORDER_PLAN`은 `WAITING/SOFT_WAIT`로 분류하며,
`RISK_BLOCKED_BY_CONDITION`과 `CONDITION_RISK_BLOCKED`는 `BLOCKING/HARD_BLOCK`으로 분류한다.
...
`stage_summary`는 현재 stage별 상태 요약이고, `stage_funnel`은 하루 단위 생존율 진단이다.
`stage_funnel.stages`는 condition hit, fusion 승격, candidate context, strategy, risk,
order plan, LIVE_SIM eligibility, intent, gateway command 순서로 `input_count`,
`survived_count`, `drop_count`, `bypass_count`, `drop_reasons`, sample id/code를 기록한다.
