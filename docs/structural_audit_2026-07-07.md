...
최종 업데이트: 2026-07-10 (PR-17 공통 market context snapshot 구현 및 격리 검증 완료)
...
- 현재 판정: MarketData PR-12와 MarketReference PR-14 장중 검증, P0-3 runtime execution lock fencing, P1-6 LIVE_SIM order-plan uniqueness, P0-4 durable broker boundary, 격리 replay, P1-2/P2-4 projection watermark/retention RCA, PR-15/16 market_index 준비와 제한적 cutover, PR-17 공통 market context 구현을 완료했다. PR-17 구현/격리 API gate는 `PASS`지만 synthetic VERIFIED fixture는 KOA Studio parser 확인이나 KRX 장중 evidence를 대체하지 않는다. 다음 구현 범위는 `market_regime` worker apply/reconcile/dry-run 준비이며 실제 cutover는 별도 PR과 KRX evidence 전까지 금지한다.

## P0

| ID | 증상 | 코드 근거 | 운영 영향 | 최소 수정 방향 | 테스트/SQL 검증 |
|---|---|---|---|---|---|
| P0-1 Gateway ingestion request path에 inline projection이 남아 있음 (부분 완화) | `api/routes/gateway.py::post_gateway_event()`는 append/outbox 기록 후 routing decision을 평가한다. `market_data`는 PR-7/9/11/12, `market_reference`는 PR-14, `market_index`는 PR-16의 guarded mode에서만 inline projection을 건너뛸 수 있다. PR-17 worker는 effective-skip index event에서 event-linked global regime과 common context pair까지 닫지만, 일반 market_regime/market_scan과 `handle_live_sim_gateway_event()`는 request path에 남아 있다. | 제한된 market_data/reference/index event는 worker로 이관할 수 있지만, 나머지 inline projection 비용과 DB write lock 경합은 여전히 Gateway POST latency와 같은 critical path에 있다. | market_regime projection, market scan, lifecycle consumer의 worker/watermark/error/retry 정책을 확정한다. 최종적으로 POST는 raw append + outbox enqueue만 수행한다. | MarketData: `docs/runbook_market_data_append_only_controller_ko.md`. MarketReference: `docs/runbook_market_reference_projection_ko.md`. MarketIndex: `docs/runbook_market_index_projection_ko.md`. MarketContext: `docs/runbook_market_context_ko.md`. SQL: `SELECT projection_name,event_type,status,COUNT(*) FROM projection_outbox GROUP BY 1,2,3;` |
...
| P1-3 MarketRegime snapshot이 공통 market context가 아니라 후보별 refresh에서 rebuild될 수 있음 | 수정 완료. schema 51의 `market_context_snapshots`는 KOSPI/KOSDAQ pair를 immutable index-event watermark hash로 묶고 `market_context_latest`가 market별 pointer를 가진다. `candidate_context_latest.market_context_snapshot_id`가 authoritative reference이며 기존 JSON은 downstream 호환 cache다. Candidate refresh와 `/api/market-regime/for-code/{code}`는 read-only resolver를 사용해 per-candidate regime snapshot을 만들지 않는다. | 같은 평가 run의 후보가 공통 market 시점과 lineage를 공유한다. membership unknown, pair missing, stale snapshot은 `DATA_WAIT`/`trading_eligible=false`이고 candidate read는 snapshot/regime count를 변경하지 않는다. | 다음 단계는 common context를 소비하는 `market_regime` projection outbox/reconcile/dry-run이며, legacy 직접 helper 제거는 해당 cutover 이후 수행한다. | 테스트: `tests/test_market_context_service.py`, `tests/test_gateway_market_index_limited_cutover.py`, `tests/test_market_open_observe_cycle.py`. API: `/api/operator/market-context/status`, `/api/operator/market-context/rebuild`. Ops: `tools/ops_market_context_check.py`. Report: `reports/market_context/20260710T055744Z/summary.md`. |
