...
최종 업데이트: 2026-07-10 (PR-27 market-index TR bootstrap/replay lineage 구현 및 격리 검증 완료)
...
- PR-27: schema 59를 유지하면서 generic `request_tr` 응답 metadata를 parent `tr_response`에 보존하고, v1 source/index/parent lineage를 만족한 KOSPI/KOSDAQ TR을 별도 `market_index` snapshot으로 projection한다. 기본은 disabled/parser unverified이며 bootstrap parent는 realtime cutover evidence와 분리돼 항상 inline-safe 경로를 유지한다. replay는 market-index table/outbox/reconcile까지 inline/worker exact parity를 검증한다.
...
- 현재 판정: MarketData PR-12와 MarketReference PR-14 장중 검증, P0-3 runtime execution lock fencing, P1-6 LIVE_SIM order-plan uniqueness, P0-4 durable broker boundary, 격리 replay, P1-2/P2-4 projection watermark/retention RCA, PR-15/16 market_index, PR-17 common context, PR-18/19 market_regime, PR-20/21 market_scan, PR-22/23 lifecycle durable consumer, PR-24 P2-1 incremental queue, PR-25 P1-1/P1-5 pipeline coherency, PR-26 P2-2 theme coherency, PR-27 P2-3 market-index TR bootstrap/replay lineage를 완료했다. 다음 범위는 최종 append-only readiness와 10거래일 evidence 판정이다. 실제 market-index/regime/scan 운영 gate는 `PENDING_KRX_SESSION`/`PENDING_KOA_STUDIO_CONFIRMATION`이며 synthetic fixture PASS로 대체하지 않는다.
...
| P2-3 Market index TR bootstrap 설정은 status에 보이지만 Core projection과 bootstrap replay 경계가 약함 | 수정 완료. generic `request_tr` command metadata가 mock/real Kiwoom 응답과 parent `tr_response`에 보존된다. `market_index_tr_bootstrap:*` request, explicit `KIWOOM_TR_BOOTSTRAP_MARKET_INDEX`, contract `v1`, TR/index/row/parser lineage가 일치한 event만 parent event ID를 sample ID로 쓰는 별도 `market_index` snapshot으로 projection한다. invalid/spoof/malformed lineage는 projection/reconcile FAIL이다. | realtime과 TR bootstrap evidence가 분리되고 KOSPI/KOSDAQ parent/sample/outbox를 event-id로 추적할 수 있다. bootstrap은 realtime cutover routing을 타지 않고 `TR_BOOTSTRAP_INLINE_ONLY`, effective skip 0을 강제한다. NXT는 KRX index evidence가 아니다. | 기본 `enabled=false`, parser `PILOT_UNVERIFIED_KOA_STUDIO`를 유지한다. KOA Studio 필드 확인과 실제 KRX 세션 전에는 운영 parser를 VERIFIED로 바꾸거나 cutover 근거로 사용하지 않는다. | 테스트: `tests/test_market_index_tr_bootstrap.py`, `tests/test_ops_market_index_tr_bootstrap_check.py`, `tests/test_projection_replay.py::test_market_index_tr_bootstrap_replay_preserves_parent_lineage`. API: `/api/operator/market-index/tr-bootstrap/*`. Report: `reports/market_index_tr_bootstrap/20260710T103254Z/summary.md`. |
...
1. 모든 consumer의 일별 reconcile/outbox/side-effect evidence를 한 read-only readiness 판정으로 묶고 현재 연속 거래일 수를 계산한다.
2. Gateway POST의 raw append + durable enqueue 기본화 가능 여부는 실제 KRX/KOA gate와 연속 10거래일 exit criteria를 함께 충족한 뒤에만 승인한다.
3. exit criteria 전에는 append-only scaffolding flag, global kill switch, emergency inline fallback을 제거하지 않는다.
...
- 기본값은 `KIWOOM_MARKET_INDEX_TR_BOOTSTRAP_ENABLED=false`, parser status `PILOT_UNVERIFIED_KOA_STUDIO`다. operator run-once는 plan-only가 기본이고 명시적 `queue_commands=true`에서도 KOSPI/KOSDAQ `request_tr`만 minute-bucket idempotency로 enqueue한다. OBSERVE가 아니거나 LIVE_SIM/LIVE_REAL 허용 시 fail-closed한다.
...
- fixture parser `VERIFIED`는 deterministic synthetic row에만 적용했고 KOA Studio 또는 실제 KRX 증거가 아니다. NXT도 KRX market-index evidence를 대체하지 않는다. Core/supervisor를 종료해 listener 0을 확인하고 fixture DB/WAL/SHM은 삭제했으며 운영 DB는 건드리지 않았다.
- bootstrap/reconcile/worker/API/ops/replay와 영향 범위 회귀를 포함한 full pytest suite `1047`개, scoped Ruff, compileall, `git diff --check`를 통과했다. 구현 gate는 `PASS`지만 실제 운영 gate는 `PENDING_KOA_STUDIO_CONFIRMATION`/`PENDING_KRX_SESSION`이다. 최종 append-only flag/inline 정리는 연속 10거래일 evidence 전까지 금지한다.

## Flag 정리 계획 (최종 순서 8)

- 현재 readiness: `BLOCKED_EVIDENCE`. 코드 준비 PR은 완료됐지만 실제 KRX/KOA gate와 연속 10거래일 evidence가 없으므로 아래 제거 작업을 실행하지 않는다.
