...
- 현재 readiness: OBSERVE-safe fixture `BLOCKED_CONFIG`, operating read-only `FAIL_SAFETY/SCHEMA_INCOMPLETE`, qualified evidence `0/10`, 운영 gate `BLOCKED_EXTERNAL_EVIDENCE`. 코드 준비와 판정기는 완료됐지만 실제 KRX/KOA gate와 연속 10거래일 evidence가 없으므로 아래 제거 작업을 실행하지 않는다.
