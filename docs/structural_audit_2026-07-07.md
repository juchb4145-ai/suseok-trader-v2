...
최종 업데이트: 2026-07-10 (PR-15 market_index worker/reconcile/dry-run 준비 완료)
...
| P1-4 Market index parser_status가 UNVERIFIED일 때 후보/Risk 정책과 운영 표시가 충분히 분리되지 않음 (부분 수정) | PR-15는 `data_usable`, `parser_status`/`parser_verified`, `data_source`를 service/reconcile/routing/Dashboard에서 별도 필드로 노출한다. metadata 없는 legacy event는 `UNKNOWN`이며 VERIFIED로 추정하지 않는다. 실제 adapter mapping은 여전히 `PILOT_UNVERIFIED_FID_MAP`이다. | projection/data usability와 adapter confidence를 구분할 수 있고, 기본 cutover readiness는 parser unverified에서 false다. 단, 공통 market_context/regime quality에 parser confidence를 연결하는 작업은 남아 있다. | KOA Studio로 FID mapping을 확인한 evidence를 남기고, 다음 market_context/regime PR에서 parser confidence warning과 trading data quality를 별도 lineage로 전달한다. | 테스트: `tests/test_market_index_projection_reconcile.py`. Report: `reports/market_index_projection_fixture/20260710T040009Z/summary.md`. |
...
1. PR-16 market_index limited cutover를 별도 구현하되 parser verified/current KRX evidence와 worker-side regime 연속성이 없으면 effective skip을 차단한다.
...
- 다음 PR 진입 판정은 `PASS`다. 이 완료는 장중 cutover나 10거래일 flag cleanup 근거를 대체하지 않는다. P1-2/P2-4 watermark/retention/RCA와 PR-15 market_index 준비까지 완료했으며 다음 구현 범위는 PR-16 limited cutover다.
...
- targeted migration/idempotency/atomicity/fail-closed/API/dashboard/ops tests와 당시 full pytest suite `895`개를 통과했다. 다음 PR 진입 판정은 `WARN_ALLOWED_RETENTION_DISABLED_SAFE_DEFAULT`였으며 이후 PR-15 market_index worker apply/reconcile 준비를 완료했다.
