...
최종 업데이트: 2026-07-10 (PR-18 market_regime worker/reconcile/dry-run 준비 및 격리 검증 완료)
...
- 현재 판정: MarketData PR-12와 MarketReference PR-14 장중 검증, P0-3 runtime execution lock fencing, P1-6 LIVE_SIM order-plan uniqueness, P0-4 durable broker boundary, 격리 replay, P1-2/P2-4 projection watermark/retention RCA, PR-15/16 market_index, PR-17 common context, PR-18 market_regime worker/reconcile/dry-run 준비를 완료했다. PR-18 격리 API gate는 `PASS`지만 synthetic VERIFIED fixture는 KOA Studio parser 확인이나 KRX 장중 evidence를 대체하지 않는다. 다음 구현 범위는 market_regime limited cutover이며 실제 skip은 별도 PR, kill switch/budget/session/health guard, KRX evidence 전까지 금지한다.

## P0

| ID | 증상 | 코드 근거 | 운영 영향 | 최소 수정 방향 | 테스트/SQL 검증 |
|---|---|---|---|---|---|
| P0-1 Gateway ingestion request path에 inline projection이 남아 있음 (부분 완화) | `api/routes/gateway.py::post_gateway_event()`는 append/outbox 기록 후 routing decision을 평가한다. `market_data`는 PR-7/9/11/12, `market_reference`는 PR-14, `market_index`는 PR-16의 guarded mode에서만 inline projection을 건너뛸 수 있다. PR-18은 일반 `market_regime` worker/reconcile/dry-run을 준비했지만 effective skip을 금지하므로 market_regime/market_scan과 `handle_live_sim_gateway_event()`는 request path에 남아 있다. | 제한된 market_data/reference/index event는 worker로 이관할 수 있지만, 나머지 inline projection 비용과 DB write lock 경합은 여전히 Gateway POST latency와 같은 critical path에 있다. | market_regime limited cutover를 독립 PR로 검증한 뒤 market scan, lifecycle consumer의 worker/watermark/error/retry 정책을 확정한다. 최종적으로 POST는 raw append + outbox enqueue만 수행한다. | MarketData: `docs/runbook_market_data_append_only_controller_ko.md`. MarketReference: `docs/runbook_market_reference_projection_ko.md`. MarketIndex: `docs/runbook_market_index_projection_ko.md`. MarketRegime: `docs/runbook_market_regime_projection_ko.md`. SQL: `SELECT projection_name,event_type,status,COUNT(*) FROM projection_outbox GROUP BY 1,2,3;` |
...
| P1-3 MarketRegime snapshot이 공통 market context가 아니라 후보별 refresh에서 rebuild될 수 있음 | 수정 완료. schema 51의 `market_context_snapshots`는 KOSPI/KOSDAQ pair를 immutable index-event watermark hash로 묶고 `market_context_latest`가 market별 pointer를 가진다. `candidate_context_latest.market_context_snapshot_id`가 authoritative reference이며 Candidate/API read는 snapshot을 생성하지 않는다. PR-18 standalone regime worker도 동일 common builder만 사용하고 superseded/retry event를 verify-only로 처리한다. | 같은 평가 run의 후보가 공통 market 시점과 lineage를 공유한다. fixture에서 첫 worker apply가 context/regime `2/1`을 만들고 idempotent retry와 superseded event에서 count가 늘지 않았다. | market_regime limited cutover 전까지 legacy inline builder를 유지한다. 후속 PR은 fresh reconcile/session/health, kill switch, 원자적 budget, 즉시 inline rollback을 모두 강제한다. | 테스트: `tests/test_market_context_service.py`, `tests/test_market_regime_projection_worker.py`, `tests/test_market_regime_projection_reconcile.py`. API: `/api/operator/market-regime-projection-reconcile/*`. Ops: `tools/ops_market_regime_projection_check.py`. Report: `reports/market_regime_projection/20260710T063020Z/summary.md`. |
...
1. 공통 `market_context` 완료 상태를 입력으로 `market_regime` worker apply/reconcile/dry-run을 준비하고 limited cutover를 별도 PR로 진행한다.
