...
최종 업데이트: 2026-07-10 (PR-16 market_index limited cutover 구현 및 격리 검증 완료)
...
- 현재 판정: MarketData PR-12와 MarketReference PR-14 장중 검증, P0-3 runtime execution lock fencing, P1-6 LIVE_SIM order-plan uniqueness, P0-4 durable broker boundary, 격리 replay, P1-2/P2-4 projection watermark/retention RCA, PR-15/16 market_index 준비와 제한적 cutover 구현을 완료했다. PR-16 격리 gate는 `PASS`지만 실제 활성화는 current KRX session, fresh heartbeat, KOA Studio parser verification evidence 전까지 금지한다. 다음 구현 범위는 공통 `market_context` snapshot과 `market_regime` worker/reconcile/cutover다.

## P0

| ID | 증상 | 코드 근거 | 운영 영향 | 최소 수정 방향 | 테스트/SQL 검증 |
|---|---|---|---|---|---|
| P0-1 Gateway ingestion request path에 inline projection이 남아 있음 (부분 완화) | `api/routes/gateway.py::post_gateway_event()`는 append/outbox 기록 후 routing decision을 평가한다. `market_data`는 PR-7/9/11/12, `market_reference`는 PR-14, `market_index`는 PR-16의 guarded mode에서만 inline projection을 건너뛸 수 있다. market_index effective skip 시 worker가 event-linked regime snapshot까지 닫지만, 일반 market_regime/market_scan과 `handle_live_sim_gateway_event()`는 request path에 남아 있다. | 제한된 market_data/reference/index event는 worker로 이관할 수 있지만, 나머지 inline projection 비용과 DB write lock 경합은 여전히 Gateway POST latency와 같은 critical path에 있다. | 공통 market context/regime, market scan, lifecycle consumer의 worker/watermark/error/retry 정책을 확정한다. 최종적으로 POST는 raw append + outbox enqueue만 수행한다. | MarketData: `docs/runbook_market_data_append_only_controller_ko.md`. MarketReference: `docs/runbook_market_reference_projection_ko.md`. MarketIndex: `docs/runbook_market_index_projection_ko.md`. SQL: `SELECT projection_name,event_type,status,COUNT(*) FROM projection_outbox GROUP BY 1,2,3;` |
...
| P1-3 MarketRegime snapshot이 공통 market context가 아니라 후보별 refresh에서 rebuild될 수 있음 (부분 완화) | PR-16 worker는 effective-skip index event마다 `source_event_id`/`source_projection=market_index` lineage를 가진 regime snapshot을 보장한다. 그러나 `services/market_regime_service.py::get_market_regime_for_code()`는 여전히 후보별 `rebuild_market_regime_snapshot()`을 호출하고 `candidate_context_latest.market_context_json`도 candidate별 저장이다. | index cutover의 worker 연속성은 보장되지만, 일반 후보 refresh 수만큼 snapshot이 생성되고 동일 평가 run 안에서 후보별 regime 시점이 달라질 수 있다. | `market_context_snapshots`를 trade_date/market/index watermark 단위로 만들고 후보 context는 snapshot id만 참조한다. KOSPI/KOSDAQ missing은 global DATA_WAIT와 per-market fallback을 구분한다. | 테스트: `tests/test_gateway_market_index_limited_cutover.py`. SQL: `SELECT target_code,COUNT(*) FROM market_regime_snapshots GROUP BY target_code;` |
| P1-4 Market index parser_status가 UNVERIFIED일 때 후보/Risk 정책과 운영 표시가 충분히 분리되지 않음 (부분 수정) | PR-15는 `data_usable`, `parser_status`/`parser_verified`, `data_source`를 별도 노출했고 PR-16은 parser verified와 realtime source를 effective cutover의 필수 fresh gate로 사용한다. metadata 없는 legacy event는 `UNKNOWN`이며 VERIFIED로 추정하지 않는다. 실제 adapter mapping은 여전히 `PILOT_UNVERIFIED_FID_MAP`이다. | projection/data usability와 adapter confidence를 구분하며 parser unverified에서는 실제 cutover가 fail-closed한다. 단, 공통 market_context/regime quality에 parser confidence를 별도 lineage로 전달하는 작업은 남아 있다. | KOA Studio로 FID mapping을 확인한 evidence를 남기고, 다음 market_context/regime PR에서 parser confidence warning과 trading data quality를 별도 lineage로 전달한다. | 테스트: `tests/test_market_index_projection_reconcile.py`, `tests/test_gateway_market_index_limited_cutover.py`. Reports: `reports/market_index_projection_fixture/20260710T040009Z/summary.md`, `reports/market_index_cutover_fixture/20260710T050254Z/summary.md`. |
...
1. 공통 `market_context` snapshot을 추가하고 `market_regime` worker/reconcile/cutover를 단계화한다.
2. market_scan worker apply/reconcile/dry-run과 limited cutover를 분리한다.
3. LIVE_SIM lifecycle을 durable idempotent consumer로 옮긴다.
4. incremental queue stale/backlog/dead-letter/retry-reset 운영 경로를 추가한다.
5. pipeline source lineage/freshness guard와 dashboard/theme coherency를 완성한다.
6. 모든 consumer 안정화 후 Gateway POST를 raw append + durable enqueue로 제한한다.
7. 연속 10거래일 exit criteria를 충족한 뒤에만 append-only scaffolding flag와 최종 inline 경로를 정리한다.
