...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check PR-8 tr_response market_data side-effect migration prep."
    )
    parser.add_argument(
        "--core-url",
    ...
    parser.add_argument(
        "--out-dir",
        default=str(ROOT_DIR / "reports" / "market_data_tr_response_side_effect"),
    )
...
def evaluate_report(report: dict[str, Any]) -> dict[str, Any]:
    ...
    tr_error_count = int(status.get("tr_response_deferred_side_effect_error_count") or 0)
    duplicate_side_effect_count = int(
        status.get("tr_response_duplicate_side_effect_count") or 0
    )
    ...
    if duplicate_side_effect_count > 0:
        failures.append("TR_RESPONSE_DUPLICATE_QUOTE_REFRESH_SIDE_EFFECT")
    if outbox_error_count > 0:
        failures.append("PROJECTION_OUTBOX_ERROR_OR_DEAD_LETTER")
    if tr_would_skip > 0 and not bool(status.get("append_only_ready")):
        failures.append("TR_RESPONSE_WOULD_SKIP_WITH_APPEND_ONLY_NOT_READY")
    if (
    ...
    if tr_would_skip == 0 and tr_deferred_count == 0:
        warnings.append("NO_TR_RESPONSE_EVENTS_OBSERVED")
    if not bool(status.get("tr_response_worker_side_effect_ready")):
        warnings.append("TR_RESPONSE_WORKER_SIDE_EFFECT_NOT_READY")
...
def render_markdown_summary(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Market Data TR Response Side Effect Check",
        "",
        f"- generated_at: `{report.get('generated_at')}`",
        f"- verdict: `{verdict.get('status')}`",
        f"- tr_response_dry_run_enabled: `{status.get('tr_response_dry_run_enabled')}`",
        f"- tr_response_cutover_enabled: `{status.get('tr_response_cutover_enabled')}`",
        _summary_line(status, "tr_response_worker_side_effect_ready"),
        _summary_line(status, "tr_response_would_skip_inline_count"),
        _summary_line(status, "tr_response_effective_skip_count"),
        _summary_line(status, "condition_event_effective_skip_count"),
        _summary_line(status, "tr_response_deferred_side_effect_count"),
        _summary_line(status, "tr_response_deferred_side_effect_error_count"),
        f"- failures: `{verdict.get('failures', [])}`",
        f"- warnings: `{verdict.get('warnings', [])}`",
        "",
        "## Safety",
        "",
        "- PR-8 prepares tr_response worker side effects only.",
        "- tr_response and condition_event inline projection remain enabled.",
...
def render_console_summary(report: dict[str, Any]) -> str:
    ...
    verdict = report.get("verdict", {})
    return (
        "market_data tr_response side-effect: "
        ...
        f"deferred={status.get('tr_response_deferred_side_effect_count')} "
        f"deferred_errors={status.get('tr_response_deferred_side_effect_error_count')}"
    )
